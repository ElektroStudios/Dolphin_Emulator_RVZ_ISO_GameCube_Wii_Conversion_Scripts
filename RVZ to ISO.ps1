...
function Convert-Files {
    ...
    foreach ($rvzFile in $rvzFiles) {    
        ...
        Write-Host " Converting $($rvzFile.FullName)..."
        ...
        switch ($exitCodeConvert) {
            0 {
                ...
                if (-not ($stdOutputVerify | Select-String "Problems Found: No")) {
                    ...
                    $outputIsoFile = New-Object System.IO.FileInfo -ArgumentList ([System.IO.Path]::ChangeExtension($rvzFile.FullName, "iso"))
                    if ($outputIsoFile.Exists) {
                        Write-Host " Press any key to delete the failed ISO file and continue converting the next file..."
                        $key = $Host.UI.RawUI.ReadKey("NoEcho, IncludeKeyDown")
                        try {
            ...
            default {
                Write-Host " Error converting $($rvzFile.FullName):" -ForegroundColor Red
                Write-Host ""
...
[System.Console]::Title = "Convert RVZ to ISO - by Elektro"
